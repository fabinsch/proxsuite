...
#include <proxsuite/linalg/qdldl/SuiteSparse_config.h>
...
void
SuiteSparse_tic(c_float tic[2] /* output, contents undefined on input */
)
{
...
int
SuiteSparse_version(int version[3])
{
  if (version != NULL) {
...
c_float
SuiteSparse_hypot(c_float x, c_float y)
{
  c_float s, r;
  x = fabs(x);
  y = fabs(y);
...
int
SuiteSparse_divcomplex(c_float ar,

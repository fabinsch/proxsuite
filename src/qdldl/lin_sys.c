...
#include <proxsuite/linalg/qdldl/qdldl_interface.h> // Include only this solver in the same directory

const char* LINSYS_SOLVER_NAME[] = { "qdldl", "mkl pardiso" };
...
c_int
load_linsys_solver(enum linsys_solver_type linsys_solver)
{
...
c_int
unload_linsys_solver(enum linsys_solver_type linsys_solver)
{
  switch (linsys_solver) {
    case QDLDL_SOLVER:
...
c_int
init_linsys_solver(LinSysSolver** s,
                   const csc* P,
                   const csc* A,
                   c_float sigma,
  ...
  switch (linsys_solver) {
    case QDLDL_SOLVER:
      return init_linsys_solver_qdldl(
        (qdldl_solver**)s, P, A, sigma, rho_vec, polish);

#ifdef ENABLE_MKL_PARDISO
    case MKL_PARDISO_SOLVER:
      return init_linsys_solver_pardiso(
        (pardiso_solver**)s, P, A, sigma, rho_vec, polish);

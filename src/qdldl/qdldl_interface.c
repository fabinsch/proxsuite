...
#include <proxsuite/linalg/qdldl/glob_opts.h>

#include <proxsuite/linalg/qdldl/qdldl.h>
...
void
free_linsys_solver_qdldl(qdldl_solver* s)
{
  if (s) {
    if (s->L)
      csc_spfree(s->L);
 ...
 */
c_int
LDL_factor(csc* A, qdldl_solver* p, c_int nvar)
{
  c_int sum_Lnz;
  c_int factor_status;
  // printf("Input matrix to LDL_factor\n");
  // for (int k = 0; k < A->n; k++)
  // {
  //     printf("A->col %lld \n",A->p[k]);
  //     printf("A->row %lld \n",A->i[k]);
  ...
  if (sum_Lnz < 0) {
    // Error
    // #ifdef PRINTING
    printf("Error in KKT matrix LDL factorization when computing the "
           "elimination tree.\n");
    if (sum_Lnz == -1) {
      printf("Matrix is not perfectly upper triangular.\n");
    } else if (sum_Lnz == -2) {
      printf("Integer overflow in L nonzero count.\n");
    }
    // #endif
    return sum_Lnz;
  }
  ...
  // printf("p->inv[0] %f \n",p->Dinv[0]);
  // printf("p->Lnz[0] %lld \n",p->Lnz[0]);
  // printf("p->etree[0] %lld \n",p->etree[0]);
  // printf("p->bwork[0] %hhu \n",p->bwork[0]);
  // printf("p->iwork[0] %lld \n",p->iwork[0]);
  // printf("p->fwork[0] %f \n",p->fwork[0]);
  // printf("");
  // printf("Input matrix before QDLDL_factor\n");
  // for (int k = 0; k < A->n; k++)
  // {
  //     printf("A->col %lld \n",A->p[k]);
  //     printf("A->row %lld \n",A->i[k]);
  //     printf("A->x[k] %f \n",A->x[k]);
  // }
  factor_status = QDLDL_factor(A->n,
                               A->p,
                               A->i,
                               A->x,
                               p->L->p,
  ...
  // printf("after QDLDL_factor %lld \n",factor_status);

  if (factor_status < 0) {
    // Error
    // #ifdef PRINTING
    printf("Error in KKT matrix LDL factorization when computing the nonzero "
            ...
            c_int* rhotoKKT)
{
  ...
  amd_status = amd_l_order(
    (*KKT)->n, (*KKT)->p, (*KKT)->i, p->P, (c_float*)OSQP_NULL, info);
#else
  amd_status =
    amd_order((*KKT)->n, (*KKT)->p, (*KKT)->i, p->P, (c_float*)OSQP_NULL, info);
#endif
  if (amd_status < 0) {
...
c_int
init_linsys_solver_qdldl(qdldl_solver** sp,
                         const csc* P,
                         const csc* A,
                         c_float sigma,
...
void
permute_x(c_int n, c_float* x, const c_float* b, const c_int* P)
{
  c_int j;
  for (j = 0; j < n; j++)
    x[j] = b[P[j]];
...
// Permute x = P'*b using P
void
permutet_x(c_int n, c_float* x, const c_float* b, const c_int* P)
{
  c_int j;
  for (j = 0; j < n; j++)
    x[P[j]] = b[j];
...
c_int
solve_linsys_qdldl(qdldl_solver* s, c_float* b)
{
  c_int j;
...
c_int
update_linsys_solver_matrices_qdldl(qdldl_solver* s, const csc* P, const csc* A)
{
...
c_int
update_linsys_solver_rho_vec_qdldl(qdldl_solver* s, const c_float* rho_vec)
{
  c_int i;

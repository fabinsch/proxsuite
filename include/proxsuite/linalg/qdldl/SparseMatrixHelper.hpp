...
namespace OsqpEigen {
/**
 * SparseMatrixHelper namespace is a namspace that contains helper function to
 * handle osqp matrix. Use it to create ot update or manage an osqp sparse
 * matrix. osqp sparse matrix in
 * [compressed-column](https://people.sc.fsu.edu/~jburkardt/data/cc/cc.html) or
 * triplet form.
 */
...
template<typename T>
bool
osqpSparseMatrixToEigenSparseMatrix(const csc* const& osqpSparseMatrix,
                                    Eigen::SparseMatrix<T>& eigenSparseMatrix);

/**
 * Convert an osqp sparse matrix into a eigen triplet list.
 * @param osqpSparseMatrix is reference to a constant pointer to a constant csc
 * struct;
...
template<typename T>
bool
osqpSparseMatrixToTriplets(const csc* const& osqpSparseMatrix,
...
#include <proxsuite/linalg/qdldl/SparseMatrixHelper.tpp>

...
#define ADAPTIVE_RHO_MULTIPLE_TERMINATION                                      \
  (4) ///< multiple of check_termination after which we update rho (if PROFILING
      ///< disabled)
#define ADAPTIVE_RHO_FIXED                                                     \
  (100) ///< number of iterations after which we update rho if termination_check
        ///< and PROFILING are disabled
#define ADAPTIVE_RHO_TOLERANCE                                                 \
  (5) ///< tolerance for adopting new rho; minimum ratio between new rho and the
      ///< current one
#endif // if EMBEDDED != 1

...
#include <stddef.h>
   ...
   * in A.  The array Ap is of size n+1, and the array Ai is of size nz = Ap
   * [n]. The matrix does not need to be symmetric, and the diagonal does not
   * need to be present (if diagonal entries are present, they are ignored
   * except for the output statistic Info [AMD_NZDIAG]).  The arrays Ai and Ap
   * are not modified.  This form of the Ap and Ai arrays to represent the
   * nonzero pattern of the matrix A is the same as that used internally by
   * MATLAB. If you wish to use a more flexible input structure, please see the
   * umfpack_*_triplet_to_col routines in the UMFPACK package, at
   * http://www.suitesparse.com.
   *
   * Restrictions:  n >= 0.  Ap [0] = 0.  Ap [j] <= Ap [j+1] for all j in the
   *       range 0 to n-1.  nz = Ap [n] >= 0.  Ai [0..nz-1] must be in the range
   * 0 to n-1.  Finally, Ai, Ap, and P must not be NULL.  If any of these
   *       restrictions are not met, AMD returns AMD_INVALID.
   *
   * AMD returns:
   *
   *       AMD_OK if the matrix is valid and sufficient memory can be allocated
   ...
   *           NULL.
   *
   *       AMD_OK_BUT_JUMBLED if the matrix had unsorted columns, and/or
   * duplicate entries, but was otherwise valid.
   *
   ...
   * modified.
   *
   *       Control [AMD_DENSE]:  controls the threshold for "dense"
   * rows/columns. A dense row/column in A+A' can cause AMD to spend a lot of
   * time in ordering the matrix.  If Control [AMD_DENSE] >= 0, rows/columns
   ...
   *           it can lead to a slightly worse ordering, however.  To turn it
   * off, set Control [AMD_AGGRESSIVE] to 0.
   *
   *       Control [2..4] are not used in the current version, but may be used
   * in future versions.
   *
   ...
   *           off-diagonal entries.  An entry A(i,j) is matched if A(j,i) is
   * also an entry, for any pair (i,j) for which i != j.  In MATLAB notation, S
   * = spones (A) ; B = tril (S, -1) + triu (S, 1) ; symmetry = nnz (B & B') /
   * nnz (B) ;
   *
   *       Info [AMD_NZDIAG]: the number of entries on the diagonal of A.
   *
   *       Info [AMD_NZ_A_PLUS_AT]:  the number of nonzeros in A+A', excluding
   * the diagonal.  If A is perfectly symmetric (Info [AMD_SYMMETRY] = 1) with a
   * fully nonzero diagonal, then Info [AMD_NZ_A_PLUS_AT] = nz-n (the smallest
   * possible value).  If A is perfectly unsymmetric (Info [AMD_SYMMETRY] = 0,
   * for an upper triangular matrix, for example) with no diagonal, then Info
   * [AMD_NZ_A_PLUS_AT] = 2*nz (the largest possible value).
   *
   *       Info [AMD_NDENSE]: the number of "dense" rows/columns of A+A' that
   * were removed from A prior to ordering.  These are placed last in the output
   * order P.
   *
   *       Info [AMD_MEMORY]: the amount of memory used by AMD, in bytes.  In
   * the current version, this is 1.2 * Info  [AMD_NZ_A_PLUS_AT] + 9*n times the
   * size of an integer.  This is at most 2.4nz + 9n.  This excludes the size of
   * the input arguments Ai, Ap, and P, which have a total size of nz + 2*n + 1
   * integers.
   *
   ...
   *           below, are also slight or rough upper bounds, for the same
   * reasons. The post-ordering of the assembly tree might also not exactly
   *           correspond to a true elimination tree postordering.
   *
   *       Info [AMD_NDIV]: the number of divide operations for a subsequent
   * LDL' or LU factorization of the permuted matrix A (P,P).
   *
   *       Info [AMD_NMULTSUBS_LDL]:  the number of multiply-subtract pairs for
   * a subsequent LDL' factorization of A (P,P).
   *
   *       Info [AMD_NMULTSUBS_LU]:  the number of multiply-subtract pairs for a
   *           subsequent LU factorization of A (P,P), assuming that no
   * numerical pivoting is required.
   *
  ...
  SuiteSparse_long amd_l_valid(SuiteSparse_long n_row,
